...
if not CAREERS_JSON.exists():
    ...
    CAREER_DATASET = [
        ...
        {"career":"Mobile Developer","skills":["swift","kotlin","react native","flutter","mobile ui design","api integration","app security"],"education":["bachelor's","bootcamp"],"description":"Build mobile applications for iOS and Android platforms."},
        {"career":"Accountant","skills":["financial accounting","tax preparation","auditing","bookkeeping","excel","quickbooks","gaap"],"education":["bachelor's","cpa"],"description":"Manage financial records and transactions."}
    ]
else:
    try:
...
if CAREER_DATASET:
    SKILL_VOCAB = sorted({s.lower() for c in CAREER_DATASET for s in c['skills']})
else:
    # Build from ESCO and O*NET
    ESCO = load_esco_occupations_and_skills(DATA_DIR / 'esco')
    ONET = load_onet_skills_and_tasks(DATA_DIR)
    combined = []
    for occ, val in ESCO.items():
        combined.append({ 'career': occ, 'skills': val.get('skills', []), 'education': [], 'description': '' })
    # Try to enrich with O*NET skills
    for occ, val in ONET.items():
        ...
        if match:
            merged = sorted({*match['skills'], *val.get('skills', [])})
            match['skills'] = [s.lower() for s in merged]
        else:
            combined.append({ 'career': occ, 'skills': [s.lower() for s in val.get('skills', [])], 'education': [], 'description': '' })
    CAREER_DATASET = combined if combined else CAREER_DATASET
...
def _derive_skills_from_quiz(answers: List[Dict[str, Any]]) -> List[str]:
    ...
    type_to_skills = {
        'A': ['python', 'analysis', 'sql', 'data visualization', 'swift', 'kotlin', 'react native', 'flutter'],
        'B': ['communication', 'teaching', 'empathy', 'presentation', 'financial accounting', 'tax preparation', 'auditing'],
        'C': ['design', 'creativity', 'ui/ux', 'writing', 'mobile ui design', 'app security'],
        'D': ['project management', 'organization', 'planning', 'leadership', 'bookkeeping', 'quickbooks', 'gaap'],
    }
...
def generate_roadmaps_for_user(user_input: Dict[str,Any]) -> Dict[str,Any]:
    ...
    try:
        ...
        pass
    # Improved career matching scoring
    scored = []
    for c in CAREER_DATASET:
        cskills = [s.lower() for s in c.get('skills', [])]
        # Calculate intersection of user skills and career skills
        matching_skills = set(skills) & set(cskills)
        # Calculate both coverage of career skills and user skills
        career_coverage = len(matching_skills) / max(1, len(cskills))
        user_skills_coverage = len(matching_skills) / max(1, len(skills))
        # Weighted average favoring career coverage slightly more
        score = (career_coverage * 0.6) + (user_skills_coverage * 0.4)
        
        # Apply bonus for keyword matches in summary
        kw_bonus = 0.15 if any(k in (summary or '').lower() for k in cskills[:5]) else 0
        # Apply bonus for education match
        edu_bonus = 0.1 if any(e.lower() in (education or '').lower() for e in c.get('education', [])) else 0
        
        final_score = score + kw_bonus + edu_bonus
        scored.append((c, final_score))
    
    # Sort by score and take the best match
    scored.sort(key=lambda x: x[1], reverse=True)
    best_career = scored[0][0] if scored else CAREER_DATASET[0]
    
    # Generate roadmaps for the best matching career
    roadmaps = core_generator({**user_input, 'skills': skills}, best_career)
    
    out = {
        'input': user_input,
        'derived_skills': skills,
        'chosen_career': best_career['career'],
        'match_score': scored[0][1] if scored else 0,
